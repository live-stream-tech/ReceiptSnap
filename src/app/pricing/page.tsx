...
const PLANS = [
  ...
  },
  {
    ...
    features: [
      ...
      { label: "収支レポート（月別グラフ・科目別集計）", included: true },
      { label: "CSVエクスポート", included: true },
      { label: "収支内訳書PDF出力", included: true },
      { label: "AI自動読み取り", included: false },
      { label: "クラウド保存（7年間）", included: false },
    ],
    cta: "PDF出力する",
  ...
  },
  {
    ...
    priceLabel: "¥1,000",
    period: "/ 回",
    description: "税務調査にも対応できる完全セット。決算書に加え、全書類を日付順に整理した証憑一覧を添付します。",
    icon: FileDown,
    ...
    features: [
      ...
      { label: "CSVエクスポート", included: true },
      { label: "収支内訳書PDF出力", included: true },
      { label: "書類を日付順に添付した証憑一覧", included: true },
      { label: "AI自動読み取り", included: false },
      { label: "クラウド保存（7年間）", included: false },
    ],
    cta: "税務調査対応で出力する",
  ...
  },
  {
    ...
    priceLabel: "¥2,500",
    period: "/ 年",
    description: "電子帳簿保存法に対応したクラウド保存。PDF出力（両プラン）・ダウンロードも料金内で何度でも。",
    icon: Cloud,
    ...
    features: [
      ...
      { label: "収支レポート（月別グラフ・科目別集計）", included: true },
      { label: "CSVエクスポート", included: true },
      { label: "PDF出力（シンプル・税務調査対応）何度でも", included: true },
      { label: "AI自動読み取り", included: false },
  ...
  },
  {
    ...
    priceLabel: "¥5,000",
    period: "/ 年",
    description: "クラウド保存・PDF出力・AI自動読み取りがすべて込み。撮るだけで書類管理が完結します。",
    icon: Sparkles,
    ...
    features: [
      ...
      { label: "CSVエクスポート", included: true },
      { label: "PDF出力（シンプル・税務調査対応）何度でも", included: true },
      { label: "AI自動読み取り（枚数無制限）", included: true },
      { label: "クラウド保存（7年間）", included: true },
    ],
    cta: "全部込みプランを始める",
    ctaHref: "/export",
    ctaStyle: "bg-gradient-to-r from-amber-500 to-orange-500 text-white hover:from-amber-600 hover:to-orange-600",
  },
];
...
const FAQS = [
  ...
  {
    q: "PDF出力（都度課金）は1回の支払いで何枚出力できますか？",
  ...
  },
  {
    ...
    a: "無料プランおよび都度課金（PDF出力）はお使いのブラウザ（端末内）に保存されます。7年保存プランは国内のセキュアなクラウドサーバーに保存され、どの端末からでもアクセスできます。",
  },
  {
    q: "返金はできますか？",
    a: "デジタルコンテンツの性質上、PDF出力の都度課金は原則として返金対応しておりません。7年保存プランは、サービス開始前であれば全額返金いたします。ご不明な点はお問い合わせください。",
  },
];
    ...
    <MainLayout>
      ...
      <div className="mb-12 animate-fade-up">
        ...
        <p className="text-lg text-brand-600/70 leading-relaxed max-w-xl">
          基本機能は無料。PDF出力は都度課金。クラウド保存・AI読み取りは年額サブスクリプション。
        </p>
      </div>

      {/* プランカード */}
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 mb-14 animate-fade-up" style={{ animationDelay: "80ms" }}>
        {PLANS.map((plan) => {
            ...
            >
              ...
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  ...
                  {"subname" in plan && plan.subname && (
                    <p className="text-xs text-brand-500/70 font-medium">{plan.subname}</p>
                  )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div className="bg-white/60 rounded-2xl p-5 border border-white/80">
            ...
            <div className="space-y-2.5">
              {[
                { label: "PDF出力（都度課金）", desc: "出力操作時に即時決済" },
                { label: "7年保存プラン", desc: "契約時に年額を一括決済。翌年同日に自動更新" },
                { label: "7年保存＋AIプラン", desc: "契約時に年額を一括決済。翌年同日に自動更新" },
              ].map(({ label, desc }) => (
                <div key={label}>
          ...
          <div className="bg-white/60 rounded-2xl p-5 border border-white/80">
            ...
            <div className="space-y-2.5">
              <div>
                <p className="text-xs font-bold text-brand-800">都度課金（PDF出力）</p>
                <p className="text-[11px] text-brand-500/70">デジタルコンテンツの性質上、原則返金不可</p>
              </div>
              <div>
                <p className="text-xs font-bold text-brand-800">7年保存プラン・AIプラン</p>
